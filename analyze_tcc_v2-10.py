...
# ── Find CSVs ─────────────────────────────────────────────────────────────────
all_csvs = [f for f in glob.glob("*.csv") if f not in ("recording_comparison.csv", "recording_summary.csv")]
seen_sizes = {}
csv_files = []
...
patient_id = input("\nEnter patient ID for this batch (e.g. P001 or 'Older Man'): ").strip() or "Unknown"
